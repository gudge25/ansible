...
dest_trusted=$(ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEST_HOST" "$SOURCE_LIST_CMD" | grep -v "trusted" | tr -d '\t')
...
for src_ip in "${source_entries[@]}"; do
    if [[ ! " ${dest_entries[*]} " =~ " ${src_ip} " ]]; then
        ...
        ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEST_HOST" "fwconsole firewall trust $src_ip"
    fi
done
...
for dest_ip in "${dest_entries[@]}"; do
    if [[ ! " ${source_entries[*]} " =~ " ${dest_ip} " ]]; then
        ...
        ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEST_HOST" "fwconsole firewall untrust $dest_ip"
    fi
done
